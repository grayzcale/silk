...
-- Wrapper function for .WaitForChild
function silk.waitFor(tree, timeout)
	for _, object in ipairs(tree) do
		if typeof(tree) == 'table' then
			tree = tree[1]
			continue
		end
		tree = tree:WaitForChild(object, timeout)
	end
	return tree
end
...
-- Check if execution is on server
function silk:Server()
	return self._server
end

-- Get service and store reference as cache

...
function network:AppendCommunicators(commDirectories)
	...
	if not self.silk:IsServer() then
		return
	end

	self._commsContainer = self._commsContainer
		or self.silk.new("Folder", self.silk.ReplicatedStorage).Name("Communicators")()
		...
		for _, comm in ipairs(directory:GetChildren()) do
			...
			for _, remoteType in ipairs({ "events", "functions" }) do
				...
				if not commData[remoteType].remotes then
					self.silk:Declare(
						error,
						`Network Error: Communicator "{comm}" was given {remoteType} but was not supplied with remotes`
					)
				end

				for _, remoteName in ipairs(commData[remoteType].remotes) do
					local remote = self.silk
						.new(remoteType == "events" and "RemoteEvent" or "RemoteFunction", self._commsContainer)
						.Name("")()
